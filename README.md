...
v0.6.0 came from a fourth review, this time across two repositories at once, which filed two defects here. The first was a seam between skills: `si-init` had learned in v0.4.0 to accept an existing system without an index or routing table, but the three consumers of that registration still assumed the tables, so the very next `si-improve` had nowhere to look. The fix is not to add the tables (v0.4.0 was right to refuse that) but a one-line **format map** in the declaration that every consumer reads, with `none` as a first-class value that routes to a fallback branch. The second was a contradiction inside `si-archive` between its description and its first step, which is now two separately measured entry triggers. The same release added a GitHub Actions workflow that parses every skill and agent frontmatter and runs `claude plugin validate --strict` on each push, because the sibling repository shipped a skill whose description was not valid YAML and nothing had said so.
