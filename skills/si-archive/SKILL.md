...
- **이동 방향은 아래로만**: 상시 로드 문서 → 경로 룰/태스크 문서(스코프 강등) · 사문화 → `docs/conventions/archive/`(로드 안 됨). **아카이브는 삭제가 아니다** — 원문·사유·일자를 보존해 "의도적 제거"와 "누락"이 구분되게 한다.
- **이동 1건 = §이관표 1행.** 원문서에는 개별 포인터를 남발하지 않는다 — 여러 조항이 나갔으면 원문서 상단에 "이관 내역: `docs/conventions/CHANGELOG.md` §이관표" 한 줄이면 충분하다.
...
grep -c '^### ' docs/conventions/CHANGELOG.md      # 로그 본문 블록 수 (캡 15)
```
...
| **사문화** | 대상 코드·절차 소멸 / 참조 0 / 도구가 이제 강제 | `docs/conventions/archive/YYYY-<slug>.md` 로 이동 + 사유 |
| **유지** | 모든 세션에서 참 | 그대로 |
...
2. 데이터 파일 §이관표에 1행: `| 일자 | 원위치 §조항 | 새 위치 | 사유(강등/병합/사문화) |`.
...
로그 블록 수 > 15 면: 오래된 본문 블록을 넘친 만큼 `docs/conventions/archive/CHANGELOG-ARCHIVE.md` **맨 위**로 이동(파일 없으면 생성). §색인 줄과 §이관표는 헤드 파일에 그대로 둔다. 이동 후 검산 재실행으로 ≤15 확인.
